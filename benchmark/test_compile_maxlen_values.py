# ruff: noqa
"""Test different max_length values to see if it's None vs not-None, or value-specific.

Run (2 nodes):
  TEST_MODE=X srun ... benchmark/test_compile_maxlen_values.py
"""
...
def main():
    ...
    maxlen_map = {"none": None, "1": 1, "1024": 1024, "16384": 16384, "32768": 32768}
    ml = maxlen_map.get(mode, None)

    kwargs = dict(
        output_dir=f"/tmp/maxlen_{mode}",
        max_steps=MAX_STEPS,
    ...
    if trainer.accelerator.is_main_process:
        print(f"[RESULT] max_length={ml}: {ms:.0f} ms/step", flush=True)

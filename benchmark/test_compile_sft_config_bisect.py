# ruff: noqa
"""Bisect which SFTConfig setting causes compile slowdown.

A: No packing args (fast baseline, ~1.6s)
B: + max_length=16384 only
C: + packing=True only
D: + max_length + packing + packing_strategy

Run (2 nodes):
  srun ... TEST_MODE=X benchmark/test_compile_sft_config_bisect.py
"""
...
def main():
    ...
    if mode == "A":
        # Baseline: no packing args
        args = SFTConfig(**base_kwargs)
    elif mode == "B":
        # + max_length only
        args = SFTConfig(**base_kwargs, max_length=SEQ_LEN)
    elif mode == "C":
        # + packing only
        args = SFTConfig(**base_kwargs, packing=True)
    elif mode == "D":
        # + all packing args
        args = SFTConfig(**base_kwargs, max_length=SEQ_LEN, packing=True, packing_strategy="wrapped")

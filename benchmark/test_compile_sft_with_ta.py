# ruff: noqa
"""Test: SFTTrainer with TrainingArguments instead of SFTConfig.

If fast → SFTConfig causes the issue
If slow → SFTTrainer class itself causes the issue

Run (2 nodes): srun ... benchmark/test_compile_sft_with_ta.py
"""
...
def main():
    ...
    # Per-layer compile manually (since TrainingArguments doesn't trigger SFT's compile)
    ...
    # Use TrainingArguments (NOT SFTConfig)
    args = TrainingArguments(
        output_dir="/tmp/sft_with_ta",
        max_steps=MAX_STEPS,
    ...
    # Use SFTTrainer class with TrainingArguments
    ...
    if trainer.accelerator.is_main_process:
        print(f"[RESULT] SFTTrainer+TrainingArguments+compile: {ms:.0f} ms/step", flush=True)

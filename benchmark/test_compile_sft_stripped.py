# ruff: noqa
"""Test: SFTTrainer with stripped compute_loss (remove entropy/accuracy overhead).

A: Full SFT compute_loss (entropy + accuracy + gather_for_metrics)
B: Stripped compute_loss (just forward + loss, like HF Trainer)
Both compiled, both max_length=16384, both with SFTTrainer + SFT collator.

Run (2 nodes): TEST_MODE=X srun ... benchmark/test_compile_sft_stripped.py
"""
...
def main():
    ...
    args = SFTConfig(
        output_dir=f"/tmp/stripped_{mode}",
        max_steps=MAX_STEPS,
    ...
    if mode == "stripped":
        # Replace SFT compute_loss with HF Trainer's default (no entropy/accuracy)
        trainer.compute_loss = Trainer.compute_loss.__get__(trainer, type(trainer))
        print("[TEST] Stripped compute_loss — using HF Trainer default", flush=True)
    else:
        print("[TEST] Full SFT compute_loss", flush=True)

# ruff: noqa
"""Test: SFTTrainer + compile but explicitly set bf16=False to prevent fp32 upcast.

Run (2 nodes): srun ... benchmark/test_compile_no_upcast.py
"""
...
def main():
    mode = os.environ.get("TEST_MODE", "with_upcast")
    ...
    if mode == "no_upcast":
        # Set bf16=False explicitly, no mixed precision -> no upcast
        args = SFTConfig(
            output_dir="/tmp/noupcast",
            max_steps=MAX_STEPS,
            ...
            tf32=True,
            bf16=False,
            fp16=False,  # explicitly disable -> mixed_precision="no"
            max_length=SEQ_LEN,
            include_num_input_tokens_seen=True,
            dataset_kwargs={"skip_prepare_dataset": True},
        )
    else:
        # Default SFTConfig (whatever triggers the upcast)
        args = SFTConfig(
            output_dir="/tmp/withupcast",
            max_steps=MAX_STEPS,
            per_device_train_batch_size=1,
            gradient_checkpointing=True,
            save_strategy="no",
    ...
    print(
        f"[TEST] mode={mode} bf16={args.bf16} fp16={args.fp16} mixed_precision={getattr(args, 'mixed_precision', '?')}",
        flush=True,
    )

    # Strip compute_loss to isolate the upcast effect

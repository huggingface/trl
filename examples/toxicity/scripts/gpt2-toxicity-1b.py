...
model = AutoModelForCausalLMWithValueHead.from_pretrained(config.model_name, torch_dtype=torch.bfloat16).to(0)
tokenizer = AutoTokenizer.from_pretrained(config.model_name)
...
# ppo_trainer = PPOTrainer(config, model, ref_model=None, tokenizer=tokenizer, dataset=dataset, data_collator=collator, num_shared_layers=4, optimizer=optimizer)
ppo_trainer = PPOTrainer(
    config, model, ref_model=None, tokenizer=tokenizer, dataset=dataset, data_collator=collator, num_shared_layers=22
)
...
ppo_trainer.save_pretrained(
    "/home/younes_huggingface_co/code/trl/examples/toxicity/scripts/gpt-neo-125m-detoxified-long-context"
)
ppo_trainer.push_to_hub("ybelkada/gpt-neo-125m-detoxified-long-context")

...
"""
python -i examples/scripts/minimal/ppo_zephyr_vllm_rm_norm.py \
    --learning_rate 3e-6 \
...
accelerate launch --config_file examples/accelerate_configs/deepspeed_zero3.7.yaml \
    examples/scripts/minimal/ppo_zephyr_vllm_rm_norm.py \
    --num_ppo_epochs 1 \
    --num_mini_batches 1 \
    --learning_rate 3e-6 \
    --output_dir models/minimal/ppo_zephyr_vllm_rm_norm3 \
    --per_device_train_batch_size 1 \
    --gradient_accumulation_steps 32 \
...
if __name__ == "__main__":
    ...
    def prepare_dataset(dataset, tokenizer):
        ...
        def tokenize(element):
            ...
            complete_input_ids = tokenizer.apply_chat_template(
                element["messages"],
                padding=False,
            )
            return {"input_ids": input_ids, "complete_input_ids": complete_input_ids}
    ...
    train_dataset = train_dataset.filter(
        ...
        and len(x["complete_input_ids"]) <= MAX_PROMPT_LENGTH + args.response_length
    )

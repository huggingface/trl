...
@torch.no_grad()
def generate(model: PreTrainedModel, batch: Dict, tokenizer: PreTrainedTokenizerBase, accelerator: Accelerator, **generation_kwargs) -> Dataset:
...
def score_and_filter(
    ...
    prompt_ids = accelerator.pad_across_processes(
                prompt_ids, dim=1, pad_index=tokenizer.pad_token_id
            )
    ...
    chosen_reward_inds = torch.argmax(rewards, dim=1) + torch.arange(0, len(all_input_ids), num_return_sequences, dtype=torch.int32, device=accelerator.device)    
    ...
    top_k_generated_texts = [
                    gen.replace(prompt, " ") for prompt, gen in zip(prompts,top_k)
                ]
        
    return Dataset.from_dict(
        {
            "prompt": prompts,
            "gen": top_k_generated_texts
        }
...
def main():
    ...
    progress_bar = tqdm(total=len(gen_dataloader)*score_args.num_raft_iterations, disable=not accelerator.is_local_main_process)
    max_length = gen_args.max_prompt_length + gen_args.max_new_tokens

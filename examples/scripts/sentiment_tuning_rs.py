...
class ScriptArguments:
    ...
    generation_n_k: Optional[int] = field(default=5, metadata={"help": "number of generations per prompt"})
...
rs_trainer = RSTrainer(config, model, tokenizer, dataset=dataset, data_collator=collator)

# We then build the sentiment analysis pipeline, passing the model name and the
# sentiment analysis pipeline arguments. Let's also make sure to set the device
# to the same device as the PPOTrainer.
device = rs_trainer.accelerator.device
if rs_trainer.accelerator.num_processes == 1:
...
for epoch, batch in tqdm(enumerate(rs_trainer.dataloader)):
    ...
    response_tensors = rs_trainer.generate(query_tensors, return_prompt=False, **generation_kwargs)
    batch["response"] = tokenizer.batch_decode(response_tensors)

    # Compute sentiment score
    n_repeats = script_args.generation_n_k
    texts = [q + r for q, r in zip([q for q in batch["query"] for _ in range(n_repeats)], batch["response"])]

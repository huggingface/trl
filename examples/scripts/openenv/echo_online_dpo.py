...
"""
Simple script to run Online DPO training with OpenEnv's Echo environment and a vLLM server. The reward function
encourages longer completions. Online DPO generates exactly 2 completions per prompt for pairwise comparison.
...
```sh
CUDA_VISIBLE_DEVICES=1 python examples/scripts/openenv/echo_online_dpo.py
```
"""
...
def rollout_func(prompts: list[str], args: OnlineDPOConfig, processing_class) -> dict[str, list]:
    """
    Custom rollout function that generates completions via vLLM server and computes environment rewards.

    For OnlineDPO, we must generate exactly 2 completions per prompt for pairwise comparison.

    Args:
        prompts: List of prompts to generate from
        args: OnlineDPOConfig containing all sampling parameters
        processing_class: Tokenizer/processor for decoding completions

    Returns:
        Dict containing prompt_ids, completion_ids, and env_reward
    """
    # 1. Generate 2 completions per prompt via vLLM inference server (running on port 8000)
    payload = {
        "prompts": prompts,
        "n": 2,  # OnlineDPO requires exactly 2 completions per prompt
    ...
    # 2. Step through the environment to get rewards for each completion
...
training_args = OnlineDPOConfig(
    output_dir="Qwen2.5-0.5B-OnlineDPO-Echo",
    vllm_mode="server",
    use_vllm=True,
    logging_steps=1,
    report_to="trackio",

...
"""AsyncGRPO training of the real Claude Code agent (loop-owning) with a LOCAL subprocess sandbox.

Local analog of claude_code_hf_sandbox.py: everything runs on one node, no remote sandbox and no tunnel.
Claude Code speaks the Anthropic Messages API only, so each rollout runs Claude Code as a local process behind
two in-process helpers: Claude Code -> Anthropic->OpenAI shim -> interception proxy (captures token_ids +
logprobs) -> your localhost vLLM. Claude Code owns its own tool loop. TRL reads the proxy trace, rebuilds
training rows, scores the workspace with a held-out verifier, and trains with GRPO.
...
agent) and returns a DENSE reward = fraction passed. `claude_code_reward` then binarizes it and adds small
degeneracy penalties. This whole file is self-contained and every training-facing object is module-level
(picklable), so the rollout worker can pickle the factory + verifier into its spawned child process.

Claude Code's runtime derives every path from an ABSOLUTE `sandbox_home` (`.claude-npm/bin/claude`, `.node`,
`workdir`, `.claude`, `proxy`, `shim`, ...). Rather than write into a real host `/home/user`, we set
`sandbox_home` to the alias `/home/user` and REMAP that prefix (in commands, paths, cwd, and harness-supplied
env values) to a per-rollout tempdir. `warmup()` installs Node 22 + `@anthropic-ai/claude-code` ONCE into a
template dir (installed at the template's real paths), and `create()` hardlink-clones the template per rollout.

Requirements:
  - An OpenAI-compatible vLLM server (see below) reachable at `--vllm-url` on localhost.
  - Internet on this node the first time: `warmup()` installs Claude Code into a template dir once.
...
# Terminal 2 - train. Proxy + shim run as local subprocesses on this node.
CUDA_VISIBLE_DEVICES=1 python examples/scripts/openenv/claude.py \
...
import claude_code_env.harness as cc_harness
from claude_code_env.claude_code_runtime import (
    build_install_cmd,
    proxy_dir,
    proxy_log_path,
    proxy_source_path,
...
from openenv.core.sandbox.base import ExecResult, SandboxHandle
...
# Alias baked into `sandbox_home`. Every Claude Code path hangs off this prefix and is remapped, per rollout, to
# the sandbox's real tempdir by LocalSandboxHandle. `/home/user` (not `/root`) so the remap can't collide with
# real host paths on a node whose user home differs.
...
# ------------------------------------------------------------------------------------------------------------
# OpenEnv's Claude Code harness only ships remote backends, and a remote sandbox can't reach a local vLLM. The
# `SandboxHandle` protocol is small, so we run Claude Code + its proxy + shim as local processes on this node.
# The harness derives every path from `config.sandbox_home`, so each sandbox REMAPS that prefix (the alias
# `/home/user`) to its own dir, and callers pass `ClaudeCodeConfig(sandbox_home="/home/user")` so config-driven
# paths funnel through the same remap.
# ============================================================================================================


class LocalBgJob:
    """A background process (the Claude Code agent, its proxy, or its shim) running directly on the node."""
    ...
    def kill(self) -> None:
        # Kill the whole process GROUP: Claude Code spawns a tree (node -> bash -> python); SIGTERM to only the
        # parent orphans the children, which pile up across rollouts. `start_bg` launches each job in its own
        # session.
...
class LocalSandboxHandle:
    ...
    def _run_env(self, envs: dict[str, str] | None) -> dict[str, str]:
        # Remap ONLY the harness-supplied env values (e.g. CLAUDE_CONFIG_DIR=/home/user/.claude). The inherited
    ...
    def start_bg(self, cmd: str, *, envs=None, cwd=None) -> LocalBgJob:
        ...
        p = subprocess.Popen(
            ...
            stdin=subprocess.DEVNULL,
            start_new_session=True,  # own process group so kill() reaps the whole Claude Code tree
        )
        job = LocalBgJob(p)
        self._bg.append(job)
        return job
...
class LocalSubprocessSandboxBackend:
    ...
    has Claude Code pre-installed (`warmup()`), so concurrent sandboxes never share state and never re-install.

    The install writes into the template at the template's REAL paths (a `ClaudeCodeConfig` copy whose
    `sandbox_home` is the template dir), so the on-disk layout matches what the runtime expects after the alias
    remap: `.claude-npm/bin/claude`, `.node`, `.node -> node-vX`, etc."""

    def __init__(self, root: str, config: ClaudeCodeConfig, *, home_alias: str = SANDBOX_HOME):
        self._root = root
        self._config = config
        self._alias = home_alias
    ...
    def warmup(self) -> None:
        """Install Node 22 + Claude Code ONCE into the template dir (run in the parent, before rollouts spawn)."""
        marker = os.path.join(self._template, ".claude-npm", "bin", "claude")
        ...
        # template (Node tarball, the `.node -> node-vX` symlink, and the `.claude-npm` npm prefix). After the
    ...
    def create(self, *, timeout_s: int = 900, envs=None, metadata=None) -> LocalSandboxHandle:
        ...
        if os.path.isdir(self._template):
            ...
            # absolute node dir, which persists), so Node + Claude Code are shared read-only across sandboxes.
            subprocess.run(["cp", "-al", f"{self._template}/.", f"{sdir}/"], check=True)
        # Reset the Claude Code per-rollout dirs (leave the install: .node, node-vX, .claude-npm).
        for sub in ("workdir", "task", "logs/agent", "logs/verifier", ".claude", "proxy", "shim"):
...
# ============================================================================================================
# Dataset + held-out verifier (identical to claude_code_hf_sandbox.py)
# ============================================================================================================
...
# ============================================================================================================
# Claude Code session factory (local sandbox + per-session proxy + shim ports)
# ============================================================================================================
...
class FreePortClaudeCodeSessionFactory(ClaudeCodeSessionFactory):
    """Same as `ClaudeCodeSessionFactory` but binds the proxy and shim to free ports per session instead of the
    hardcoded `_PROXY_PORT = 7000` / `_SHIM_PORT = 7100`, so several Claude Code sandboxes can run at once on one
    node. Mirrors OpenEnv's `_start_proxy` / `_start_shim` exactly except for the ports. The proxy + shim run as
    local subprocesses under the SYSTEM python (the trainer env already has fastapi/uvicorn/httpx via vLLM), so
    no in-template pip install is needed."""
    ...
    def _start_proxy(self, sandbox):
        ...
        if not sandbox.exists(proxy_source_path(self._config)):
            sandbox.write_text(proxy_source_path(self._config), cc_harness._PROXY_SOURCE_PATH.read_text())
    ...
    def _start_shim(self, sandbox, upstream_url):
        port = _free_port()
        if not sandbox.exists(shim_source_path(self._config)):
            sandbox.write_text(shim_source_path(self._config), cc_harness._SHIM_SOURCE_PATH.read_text())
            sandbox.write_text(f"{shim_dir(self._config)}/__init__.py", "")

        shim_args = [
            "python", "anthropic_shim.py", "--upstream-url", upstream_url,
            "--api-key", self._config.api_key, "--port", str(port),
        ]  # fmt: skip
        quoted = " ".join(shlex.quote(a) for a in shim_args)
        shim_cmd = (
            f"cd {shlex.quote(shim_dir(self._config))} && {quoted} > {shlex.quote(shim_log_path(self._config))} 2>&1"
        )
        ...
        for _ in range(120):
            ...
            try:
                ...
                pass
            shim_job.kill()
            raise RuntimeError(f"shim did not start on :{port}\n{log[-2000:]}")

        # Claude Code appends /v1/messages, so hand it the shim root.
        return shim_job, f"http://127.0.0.1:{port}"


def build_factory(
    sandbox_root: str, vllm_url: str, model: str, tests_by_id: dict, max_turns: int
    ...
    config = ClaudeCodeConfig(
        ...
        tools=ALLOWED_TOOLS,
        max_turns=max_turns,
        # Cap forwarded max_tokens well below max_model_len so Claude Code's large, turn-growing
        # prompt + max_tokens never exceeds the vLLM context window (which 400s the request).
        proxy_max_tokens_cap=8192,
    )
    backend = LocalSubprocessSandboxBackend(sandbox_root, config)
    backend.warmup()  # install Node 22 + Claude Code ONCE (parent, before rollouts)
    inner = FreePortClaudeCodeSessionFactory(
        config=config,
...
def main() -> None:
    ...
    p.add_argument("--max-inflight", type=int, default=8)  # concurrent rollouts (each its own sandbox + ports)
    ...
    p.add_argument("--output-dir", default="async_grpo_claude_code")
    p.add_argument("--project", default="claude-code")
    ...
    sandbox_root = args.sandbox_root or tempfile.mkdtemp(prefix="trl_claude_code_")
    ...
    worker = HarnessRolloutWorker(
        harness_session_factory=build_factory(sandbox_root, args.vllm_url, args.model, tests_by_id, args.max_turns),

...
"""AsyncGRPO training of the real Pi coding agent (loop-owning) in REMOTE Hugging Face sandboxes.

Same loop-owning path as opencode_hf_sandbox.py, for the `pi` coding-agent CLI. Pi speaks the OpenAI API
natively (no translation shim), so each rollout runs Pi in a remote HF sandbox behind an in-sandbox proxy
(`transparent_proxy` mode): Pi -> interception proxy (captures token_ids + logprobs) -> vLLM. Pi owns its own
tool loop; TRL reads the proxy trace, scores the workspace with a held-out verifier, and trains with GRPO.
...
from pi_env import PiConfig, PiSessionFactory, PiTask
...
# No pre-baked Pi image yet, so cold-install per rollout on a plain base. The HF sandbox execs as root, so every
# harness path hangs off /root.
...
# ============================================================================================================
# Pi session factory (remote HF sandbox + in-sandbox proxy)
# ============================================================================================================
...
def build_factory(sandbox_vllm_url: str, model: str, tests_by_id: dict, image: str, flavor: str) -> PiTaskFactory:
    config = PiConfig(
        base_url=f"{sandbox_vllm_url}/v1",  # the in-sandbox proxy forwards here (public url / tunnel)
...
def main() -> None:
    ...
    p.add_argument("--output-dir", default="async_grpo_pi_hf_sandbox")
    p.add_argument("--project", default="pi-hf-sandbox")

...
```sh
python examples/grpo_sudoku/sudoku.py --vllm-mode colocate
```
...
```sh
CUDA_VISIBLE_DEVICES=1 python examples/grpo_sudoku/sudoku.py --vllm-mode server --vllm-server-url http://localhost:8000
```
...
```sh
python examples/grpo_sudoku/sudoku.py --env-mode docker-local --vllm-mode colocate
```

# Full example with all flags:
```sh
python examples/grpo_sudoku/sudoku.py \
    --vllm-mode colocate \
    --env-mode space \

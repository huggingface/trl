...
class TestRLOOTrainer(TrlTestCase):
    ...
    def test_flush_user_logs_asymmetric_metrics_multi_process(self):
        ...
        for pending in ranks_pending:
            ...
            trainer._flush_user_logs = RLOOTrainer._flush_user_logs.__get__(trainer)
        ...
        for trainer in trainers:
            ...
            with patch("trl.trainer.rloo_trainer.gather_object", side_effect=mock_gather_obj):
    ...
    def test_flush_user_logs_weighted_averaging_unequal_sample_counts(self):
        ...
        with patch("trl.trainer.rloo_trainer.gather_object", side_effect=lambda obj: obj):
            trainer._flush_user_logs(prompts_text=["prompt"], mode="train", device=torch.device("cpu"))
    ...
    def test_flush_user_logs_empty(self):
        ...
        with patch("trl.trainer.rloo_trainer.gather_object", return_value=[]):

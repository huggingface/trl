...
class TestChunkedCrossEntropyLoss:
    ...
    @staticmethod
    def _reference(hidden, weight, labels, num_items_in_batch=None, ignore_index=-100):
        shift_h = hidden[..., :-1, :].reshape(-1, hidden.size(-1))
        shift_l = labels[..., 1:].reshape(-1)
        logits = shift_h.float() @ weight.float().t()
        if num_items_in_batch is None:
            loss = F.cross_entropy(logits, shift_l, ignore_index=ignore_index, reduction="mean")
        else:
            loss = F.cross_entropy(logits, shift_l, ignore_index=ignore_index, reduction="sum")
            loss = loss / num_items_in_batch
        valid = shift_l != ignore_index
        if valid.any():
            log_p = F.log_softmax(logits, dim=-1)
    ...
    def test_forward_matches_cross_entropy(self):
        ...
        loss_c, acc_c, ent_c = chunked_cross_entropy_loss(hidden, weight, labels, chunk_size=self.CHUNK_SIZE)
    ...
    def test_num_items_in_batch_reduction(self):
        ...
        loss_c, *_ = chunked_cross_entropy_loss(
    ...
    def test_backward_matches_reference(self):
        ...
        loss_c, *_ = chunked_cross_entropy_loss(hidden_c, weight_c, labels, chunk_size=self.CHUNK_SIZE)
        loss_c.backward()
    ...
    def test_all_ignored_returns_zero(self):
        ...
        hidden, weight, labels = self._inputs()
        labels[:] = -100
        loss, acc, ent = chunked_cross_entropy_loss(hidden, weight, labels, chunk_size=self.CHUNK_SIZE)
...
class TestPatchChunkedCELMHead:
    ...
    def _setup(self, model_id):
        ...
        patch_chunked_ce_lm_head(chunked_model, chunk_size=self.CHUNK_SIZE)

...
class PPOTrainerTester(unittest.TestCase):
    ...
    def test_drop_last_dataloader(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            ppo_config = PPOConfig(output_dir=tmp_dir, batch_size=3, mini_batch_size=1, log_with=None)
    ...
    def test_ppo_step_with_no_ref_sgd_lr_scheduler(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            assert train_stats["ppo/learning_rate"] > ppo_config.learning_rate
    ...
    def test_ppo_step_no_dataset(self):
        ...
        query_tensor = self.gpt2_tokenizer.encode(query_txt, return_tensors="pt")
        with tempfile.TemporaryDirectory() as tmp_dir:
            ppo_config = PPOConfig(output_dir=tmp_dir, batch_size=1, mini_batch_size=1, log_with=None)
    ...
    def test_ppo_trainer_max_grad_norm(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ppo_config = PPOConfig(
                output_dir=tmp_dir, batch_size=2, mini_batch_size=1, max_grad_norm=0.00001, log_with=None
            )
            ppo_trainer = PPOTrainer(
                config=ppo_config,
                model=self.gpt2_model,
            ...
            for name, param in ppo_trainer.model.named_parameters():
                assert param.grad is not None, f"Parameter {name} has no gradient"
                assert torch.all(
                    param.grad.abs() <= ppo_config.max_grad_norm
    ...
    def test_ppo_trainer_kl_penalty(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            ppo_config = PPOConfig(
                output_dir=tmp_dir, batch_size=2, mini_batch_size=1, kl_penalty="abs", log_with=None
            )
            ppo_trainer = PPOTrainer(
                config=ppo_config,
                model=self.gpt2_model,
    ...
    def test_ppo_trainer_full_kl_penalty(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ppo_config = PPOConfig(
                output_dir=tmp_dir, batch_size=2, mini_batch_size=1, kl_penalty="full", log_with=None
            )
            ppo_trainer = PPOTrainer(
                config=ppo_config,
                model=self.gpt2_model,
    ...
    def test_generation(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            generations_single = [
                ppo_trainer.generate(inputs, **generation_kwargs).squeeze() for inputs in model_inputs
            ]
    ...
    def test_generation_with_ref_model(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            for inputs in model_inputs:
                generation, ref_generation = ppo_trainer.generate(
                    inputs, generate_ref_response=True, **generation_kwargs
                )
    ...
    def test_grad_accumulation(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ppo_config = PPOConfig(output_dir=tmp_dir, batch_size=2, mini_batch_size=2, ppo_epochs=1, log_with=None)
            ppo_trainer = PPOTrainer(
                config=ppo_config,
                model=gpt2_model,
                ref_model=None,
    ...
    def test_batch_size_check(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            with pytest.raises(ValueError):
                PPOConfig(output_dir=tmp_dir, batch_size=2, mini_batch_size=2, gradient_accumulation_steps=2)

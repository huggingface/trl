...
class TestOpenRewardSpec(TrlTestCase):
    ...
    def test_unrewarded_rollout_returns_none_not_zero(self, echo_env_url):
        ...
        unscorable_mask zero the advantage, carrying no gradient. (Fixes #7364; reimplements #6430
        scoped to OpenReward only.)
        """
        spec = OpenRewardSpec(echo_env_url, env_name="echoenvironment", num_tasks=1)
        env = spec.environment_factory()
        env.reset(**spec.train_dataset[0])
    ...
    def test_correct_echo_sets_has_reward(self, echo_env_url):
        spec = OpenRewardSpec(echo_env_url, env_name="echoenvironment", num_tasks=1)
        env = spec.environment_factory()
        env.reset(**spec.train_dataset[0])
        assert env.has_reward is False
        env.echo(text="hello")
        assert env.has_reward is True
        assert env.reward == 1.0
        assert spec.reward_funcs(environments=[env]) == [1.0]
        env._close()

...
from trl import SRPOConfig, SRPOTrainer, FDivergenceType

from .testing_utils import require_bitsandbytes, require_no_wandb, require_peft


class SRPOTrainerTester(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        ...
        cls.tokenizer.chat_template = """Below is a reddit POST and the corresponding SUBREDDIT and TITLE{{", and an EXAMPLE SUMMARY." if example else "."}}
...
"""
        ...
        # cls.t5_tokenizer.add_special_tokens({"pad_token": "[PAD]"})
        cls.t5_tokenizer.chat_template = """Below is a reddit POST and the corresponding SUBREDDIT and TITLE{{", and an EXAMPLE SUMMARY." if example else "."}}
Write a both precise and concise summary of the contents of the POST.
{{messages}}
{%- if example %}
EXAMPLE SUMMARY: {{example + "\n"}}
...
"""
    ...
    def test_srpo_trainer(self, name, pre_compute):
    ...
    def test_srpo_trainer_without_providing_ref_model(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            training_args = SRPOConfig(
                output_dir=tmp_dir,
                per_device_train_batch_size=2,
    ...
    @require_peft
    def test_srpo_trainer_without_providing_ref_model_with_lora(self):
    ...
    def test_srpo_trainer_padding_token_is_none(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            training_args = SRPOConfig(
                output_dir=tmp_dir,
                per_device_train_batch_size=2,
    ...
    def test_srpo_trainer_w_dataset_num_proc(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            training_args = SRPOConfig(
                output_dir=tmp_dir,
                per_device_train_batch_size=2,
    ...
    @require_no_wandb
    def test_srpo_trainer_generate_during_eval_no_wandb(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            training_args = SRPOConfig(
                output_dir=tmp_dir,
                per_device_train_batch_size=2,
    ...
    def test_srpo_lora_bf16_autocast_llama(self):
...
"""
    ...
    def test_srpo_lora_bf16_autocast(self, name, pre_compute, gen_during_eval):
        # Note this test only works on compute capability > 7 GPU devices
        from peft import LoraConfig
...
"""
    ...
    @require_peft
    def test_srpo_lora_force_use_ref(self):
        from peft import LoraConfig, get_peft_model
    ...
    def test_srpo_trainer_torch_dtype(self):
        # See https://github.com/huggingface/trl/issues/1751
        dummy_dataset = self._init_dummy_dataset()
        with tempfile.TemporaryDirectory() as tmp_dir:
            dpo_config = SRPOConfig(
                output_dir=tmp_dir,
                per_device_train_batch_size=2,
                max_steps=1,
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            with pytest.raises(
                ValueError,
                match="Invalid `torch_dtype` passed to the SRPOConfig. Expected a string with either `torch.dtype` or 'auto', but got -1.",
            ):
                _ = SRPOTrainer(
                    model=self.model_id,
                    tokenizer=self.tokenizer,

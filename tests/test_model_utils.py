...
def test_prepare_deepspeed_strips_optimizer_for_cpu_offload(stage):
    # prepare_deepspeed initializes eval-only models without an optimizer. If the deep-copied config still carries
    # `optimizer` + `offload_optimizer: cpu`, DeepSpeed builds a CPUAdam on GPU params and the run dies before training.
    # Both keys must be dropped before `deepspeed.initialize`. Intercept the call to check the config it would receive.
    ds_config = {

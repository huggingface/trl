...
class TestGRPOTrainer(TrlTestCase):
    def get_vllm_lora_sync_args(self, **kwargs):
        defaults = {
    ...
    def test_vllm_lora_sync_config_default(self):
        assert GRPOConfig(output_dir=self.tmp_dir).vllm_lora_sync is False

    @require_peft
    def test_vllm_lora_sync_requires_use_vllm(self):
        training_args = self.get_vllm_lora_sync_args(use_vllm=False)
        with pytest.raises(ValueError, match="requires `use_vllm=True`"):
            GRPOTrainer(
    ...
    @require_peft
    def test_vllm_lora_sync_requires_server_mode(self):
        training_args = self.get_vllm_lora_sync_args(vllm_mode="colocate")
        with pytest.raises(ValueError, match="supported only with `vllm_mode='server'`"):
            GRPOTrainer(
                model="trl-internal-testing/tiny-Qwen2ForCausalLM-2.5",
                reward_funcs=self.reward_func,
                args=training_args,
    ...
    def test_vllm_lora_sync_requires_peft_model(self):
        training_args = self.get_vllm_lora_sync_args()
    ...
    @require_peft
    def test_vllm_lora_sync_is_passed_to_vllm_generation(self):
        training_args = self.get_vllm_lora_sync_args()
        with (
        ...
        assert mock_vllm_generation.call_args.kwargs["lora_sync"] is True
    ...
    def test_train_scale_rewards(self, scale_rewards):
        dataset = load_dataset("trl-internal-testing/zen", "standard_prompt_only", split="train")
    ...
    def test_train_sequence_importance_sampling(self):
        dataset = load_dataset("trl-internal-testing/zen", "standard_prompt_only", split="train")

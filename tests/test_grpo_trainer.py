...
class TestGRPOTrainer(TrlTestCase):
    ...
    def test_compute_liger_loss_dapo_normalizer(self):
        ...
        # Capture (num_items_in_batch kwarg, raw loss) for every Liger loss call ...
        raw_calls = []
        original_forward = trainer.liger_grpo_loss.forward
        ...
        # ... and the corresponding loss returned by compute_liger_loss (post-normalizer).
        final_losses = []
        ...
        with (
            patch.object(trainer.liger_grpo_loss, "forward", side_effect=forward_capture),
            patch.object(trainer, "compute_liger_loss", side_effect=cll_capture),
        ):
            trainer.train()

        assert len(raw_calls) > 0, "liger_grpo_loss.forward was never called"
        assert len(final_losses) == len(raw_calls)
        for (num_items, raw_loss), final_loss in zip(raw_calls, final_losses, strict=True):
            # (1) num_items_in_batch must be forwarded so the Liger DAPO normalizer matches TRL
            assert num_items is not None, "num_items_in_batch should be passed to liger_grpo_loss for dapo"
            # (2) no division by gradient_accumulation_steps for dapo/cispo/vespo

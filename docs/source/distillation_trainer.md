...
- Teachers must be unquantized teachers loaded without device dispatch or offload hooks. Teachers move between CPU and the accelerator as complete modules, and the loss reads their output heads as dense matrices; quantized and dispatched teacher backends have not been integrated or validated. Active quantization (`load_in_8bit`, `load_in_4bit`, a `quantization_config`) and an active `device_map` are rejected at initialization, as is an already-instantiated teacher that is quantized or dispatched. `device_map=None` is allowed.

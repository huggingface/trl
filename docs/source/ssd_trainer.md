...
Use [`examples/ssd_codegen/ssd_eval.py`](https://github.com/huggingface/trl/blob/main/examples/ssd_codegen/ssd_eval.py) to evaluate a base model or an SSD-trained checkpoint on LiveCodeBench v6. The script uses vLLM for generation and LiveCodeBench's official `codegen_metrics` for sandboxed `pass@k` scoring; default decoding parameters match Table 3 of the paper.

```bash
python examples/ssd_codegen/ssd_eval.py \

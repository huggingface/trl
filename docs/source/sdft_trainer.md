...
Use [`examples/sdft_privileged_context/sdft.py`](https://github.com/huggingface/trl/blob/main/examples/sdft_privileged_context/sdft.py) to launch SDFT training from the command line. The script supports any causal LM from the Hub, custom local datasets via `--dataset_path`, and PEFT/LoRA via the standard `ModelConfig` flags.

...
![](https://huggingface.co/datasets/trl-internal-testing/example-images/resolve/main/images/online-dpo-qwen2.png) # TODO: update the image
...
#TODO: fill the demo below when the training is done.
...
Instead of a judge, you can chose to use a reward model to evaluate the completions to leverage the capabilities of reward model -- see [Reward Bench](https://huggingface.co/spaces/allenai/reward-bench) for a leaderboard of public models you can use. The following is a code example with [trl-lib/Qwen2-0.5B-Reward](https://huggingface.co/trl-lib/Qwen2-0.5B-Reward) as the reward model:

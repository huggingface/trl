...
2. Concurrently, we estimate the KL divergence between the sampling policy  \\( \pi_{\text{s}} \\) and the fixed reference policy  \\( \pi_{\text{ref}} \\) over the sequence. We use the lower-variance [k3 estimator](http://joschu.net/blog/kl-approx.html):

$$
\widehat{\mathbb{D}}_{\mathrm{KL}}\!\left[\pi_{\text{s}}\|\pi_{\mathrm{ref}}\right]
= \sum_{t=1}^T \left[
\frac{\pi_{\mathrm{ref}}(o_{i,t} \mid q, o_{i,<t})}{\pi_{\text{s}}(o_{i,t} \mid q, o_{i,<t})}
- \log \frac{\pi_{\mathrm{ref}}(o_{i,t} \mid q, o_{i,<t})}{\pi_{\text{s}}(o_{i,t} \mid q, o_{i,<t})} - 1
\right].
$$
...
$$
r_i = R(o_i, q) - \beta \, \widehat{\mathbb{D}}_{\mathrm{KL}}\!\left[\pi_{\text{s}} \|\pi_{\mathrm{ref}}\right],
$$

where  \\( \beta > 0 \\) controls the strength of the KL penalty.

> [!TIP]
> In a purely online setting (`num_iterations = 1`, default), the data are generated by the current policy, so  \\( \pi_{\text{s}} = \pi_\theta \\).
>
> In the more general setting (e.g., multiple gradient steps per batch), the data are generated by an earlier snapshot, so  \\( \pi_{\text{s}} = \pi_{\text{old}} \\).

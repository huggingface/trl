...
1. **Input arguments**:
   - The function must accept the following as keyword arguments:
     ...
     - `log_extra`: a callable `log_extra(column: str, values: list)` to add extra columns to the completions table. See Example 6. In distributed training, processes may log different columns; a column a process did not log is padded with `None` for that process's completions.
     - `log_metric`: a callable `log_metric(name: str, value: float)` to log scalar metrics as plots alongside `kl`, `entropy`, etc. See Example 6. In distributed training, processes may log different metrics and different numbers of values; each metric is averaged over all the values logged across processes.

...
- `policy_loss`: The clipped surrogate objective, logged on every run. It excludes the Mixture-of-Experts auxiliary loss, which is added afterwards, and carries no KL term because RLOO folds the KL penalty into the reward rather than the loss.

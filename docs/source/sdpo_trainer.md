...
Use [`examples/sdpo_math/sdpo.py`](https://github.com/huggingface/trl/blob/main/examples/sdpo_math/sdpo.py) to launch SDPO training from the command line. The script supports verifiable math rewards, environment feedback via `--feedback_column`, and PEFT/LoRA via the standard `ModelConfig` flags.

```bash
python examples/sdpo_math/sdpo.py \

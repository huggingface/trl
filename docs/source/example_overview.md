...
| [`sft_redacted`](https://github.com/huggingface/trl/tree/main/examples/sft_redacted) | LoRA SFT of REDACTED on 8 nodes with [`SFTTrainer`], sharded at load time by transformers' `DistributedConfig` (expert parallelism with token dispatch). | |

...
| [`examples/scripts/privileged_gkd.py`](https://github.com/huggingface/trl/blob/main/examples/scripts/privileged_gkd.py) | 该脚本展示如何使用 [`experimental.gkd.PrivilegedSelfDistillTrainer`]，基于特权信息（`privileged_messages`）进行 on-policy 自蒸馏。 |
| [`examples/scripts/privileged_gkd_smoke.py`](https://github.com/huggingface/trl/blob/main/examples/scripts/privileged_gkd_smoke.py) | 该脚本提供 [`experimental.gkd.PrivilegedSelfDistillTrainer`] 的最小单步 smoke 运行示例（内置内存数据集）。 |

...
| [`async_grpo_prophet_arena`](https://github.com/huggingface/trl/tree/main/examples/async_grpo_prophet_arena) | AsyncGRPO training of a binary-market forecaster on [Prophet Arena](https://www.prophetarena.co) with a Brier-score reward, ported from thinking-machines-lab/tinker-cookbook's `recipes/forecasting`. | |
| [`async_grpo_timesx`](https://github.com/huggingface/trl/tree/main/examples/async_grpo_timesx) | AsyncGRPO training of a numeric time-series forecaster on the [TimesX](https://huggingface.co/papers/2607.06973) evaluation benchmark, with a MASE-based reward. | |
...
| [`sft_qwen3_8b_1m_context`](https://github.com/huggingface/trl/tree/main/examples/sft_qwen3_8b_1m_context) | SFT of Qwen3-8B on 1,048,576-token sequences with context parallelism, on one 8xH100 node. | |
...
| [`tail_sft_gsm8k`](https://github.com/huggingface/trl/tree/main/examples/tail_sft_gsm8k) | TailSFT on GSM8K by extending [`SFTTrainer`] with initial-policy loss filtering. | |

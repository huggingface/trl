...
| [`async_grpo_mini_swe_agent`](https://github.com/huggingface/trl/tree/main/examples/async_grpo_mini_swe_agent) | LoRA AsyncGRPO training of the real `mini-swe-agent` coding agent on SWE-Gym (loop-owning: the agent runs its own bash loop in a Hugging Face sandbox started from the instance's image, TRL trains on its recorded turns, SWE-bench's tests are the reward), with launch scripts for a disaggregated Hugging Face Jobs setup where the adapter reaches the vLLM Job through a Storage Bucket. | |

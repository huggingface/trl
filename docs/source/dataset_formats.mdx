...
Some trainers also support fine-tuning vision language models (VLMs) on image-text pairs. In this case, the dataset must contain the key `images`, and the trainer's `tokenizer` is the VLM's `processor`. An example of such a dataset is the [openbmb/RLAIF-V-Dataset](https://huggingface.co/datasets/openbmb/RLAIF-V-Dataset) dataset. We provide this dataset in the conversational format:

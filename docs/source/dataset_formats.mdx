...
<table>
 ...
 "steps": ["Let a represent the price of an apple.", "Let b represent the price of an orange"],
 "labels": ["True", "True"]}</code></pre>
    </td>
    <td>
...
A steps preference dataset is similar to an unpaired preference dataset but instead of having a single `"completion"` and `"label"`, it includes a `"stepwise_completion"` column that splits the completion into a list of steps and a `"stepwise_labels"` indicating whether each step is correct or not.
...
| [`StepwiseRewardTrainer`] | Steps preference             |

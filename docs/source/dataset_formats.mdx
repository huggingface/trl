...
To convert a language modeling dataset into an unpaired preference dataset, process the two text datasets into a completion with a label and concatenate.
...
data_as_unpaired_preference = concatenate_datasets([transformed_good_examples, transformed_bad_examples])

...
| ------------------------------- | ----------------------------------------------------------------------- | ----------------------------------------------------------------------- | ----------------------------------------------------------------- | --------------------------------------------------------- | --------------------------------------------------------- | ------------------------------------------------------------------------- | -------------------- |
| Language modeling               | N/A                                                                     | N/A                                                                     | N/A                                                               | N/A                                                       | N/A                                                       | [🔗](#from-language-modeling-to-unpaired-preference-dataset)                                                                       | N/A                  |
...
### From language modeling to unpaired preference dataset

To convert two language modeling datasets into an unpaired preference dataset, process the text from each data into a completion, give each a label and an empty prompt, and concatenate.


```python
from datasets import Dataset, concatenate_datasets

def process_into_preference(label:bool, completion_column:str='text'):
    def transform_row(example):
        return {"prompt": "", "completion":example[completion_column], 'label':label}
    return transform_row

good_examples = Dataset.from_dict({
    "text": ["The sky is blue.", "Grass is green."],
})
bad_examples = Dataset.from_dict({
    "text": ["Give me your wallet", "F=ma^2"],
})

transformed_bad_examples = bad_examples.map(process_into_preference(False), remove_columns=['text'])

transformed_good_examples = good_examples.map(process_into_preference(True), remove_columns=['text'])

dataset = concatenate_datasets([transformed_good_examples, transformed_bad_examples])

```

```python
>>> dataset[0]
{'prompt': '', 'completion': 'The sky is blue.', 'label': True}
```

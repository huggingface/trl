...
The standard dataset format typically consists of plain text strings. The columns in the dataset vary depending on the task. This is the format expected by TRL trainers.

### Conversational dataset format

Conversational datasets are used for tasks involving dialogues or chat interactions between users and assistants. Unlike standard datasets format, these contain sequences of messages where each message has a `role` (e.g., `"user"` or `"assistant"`) and `content` (the message text). Conversational datasets are useful for training chat-based models but must be converted into a standard format before being used with TRL trainers. This is typically done using chat templates specific to the model being used. For more information, refer to the [Working with conversational datasets in TRL](#working-with-conversational-datasets-in-trl) section.
...
> ```
>
> - The prompt-only output includes a placeholder for the assistant’s response, indicated by `'<|assistant|>\n'`, expecting a continuation.
> - In contrast, the language modeling output treats the input as a full sequence and terminates it with `'<|endoftext|>'`, without expecting an additional response.
...
The dataset format you should use depends on the task you're working on. Here's a brief overview of the dataset formats supported by each TRL trainer:
...
Conversational datasets are increasingly common, especially for training chat models. However, TRL trainers don't support conversational datasets in their raw format. These datasets must first be converted into a standard format. 
Fortunately, TRL offers tools to easily handle this conversion, which are detailed below.

### A brief introduction to conversational datasets

A conversational dataset is structured as a list of messages, unlike standard datasets which are typically formatted as single strings. These datasets are particularly useful for training conversational models, as they more accurately represent the flow of a conversation.

In a conversational dataset, each message has a role (e.g., `"user"` or `"assistant"`) and content (e.g., `"Hello, how are you?"`). Here's an example:
...
A conversational dataset is not meant to be used directly with TRL trainers. Instead, it should be converted into a standard dataset format before training. This conversion is done by applying a chat template to the dataset.

### Converting a conversational dataset into a standard dataset

TRL trainers do not support conversational datasets in their raw format. To use them, you need to convert them into a standard dataset format using a chat template. This template is provided by the tokenizer of the model you plan to use.

For detailed instructions on using chat datasets, refer to the [Chat templating section in the `transformers` documentation](https://huggingface.co/docs/transformers/en/chat_templating).
...
It's important to note that chat templates are model-specific. For example, if you use the chat template from the [meta-llama/Meta-Llama-3.1-8B-Instruct](https://huggingface.co/meta-llama/Meta-Llama-3.1-8B-Instruct) model, you get a different structure:
...
> [!WARNING]
> Always use the chat template associated with the model you're working with. Chat templates are model-specific, and using the wrong template can lead to inaccurate or unexpected results.

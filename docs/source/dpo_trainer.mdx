...
[`DPOTrainer`] can be used to fine-tune vision language models (VLMs). In this case, the dataset must also contain the key `images`, and the trainer's `tokenizer` is the VLM's `processor`. For example, for Idefics2, the processor expects the dataset to have the following format:
...
dpo_trainer = DPOTrainer(
    ...
    tokenizer=tokenizer,  # for vision language models, use tokenizer=processor instead
)
```

...
where the `prompt` contains the context input, `response` contains an array of at least two responses and `pairs` contains a list of positive (accepted) and negative (rejected) indices from the corresponding `responses` where the first index is the positive response and the second index is the negative one. As can be seen a prompt can have multiple responses and this is reflected in the `pairs` entry.

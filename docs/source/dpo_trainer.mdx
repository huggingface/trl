...
Given the preference data, we can fit a binary classifier according to the Bradley-Terry model and in fact the DPO authors propose to use the sigmoid loss on the normalized likelihood via the `logsigmoid` to fit a logistic regression.
 loss on the normalized likelihood in the [DSO](https://arxiv.org/abs/2309.06657) paper. The `DPOTrainer` can be switched to this loss via the `loss_type="hinge"` argument and the `beta` in this case is the reciprocal of the margin.
The [SLiC](https://arxiv.org/abs/2305.10425) authors propose to use a hinge

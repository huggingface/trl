...
We provide an example of how to extend the `SFTTrainer` for a Vision Language Models in the example [`examples/scripts/vsft_llava.py`](https...). This script involves using a custom collator that we tweaked to handle the data data processing as well.

### Defining a custom collator

```python
def collate_fn(examples):
    # Get the texts and images, and apply the chat template
    texts = [processor.tokenizer.apply_chat_template(messages, tokenize=False, add_generation_prompt=False) for example in examples]

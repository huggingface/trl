...
First install `unsloth` according to the official documentation. Once installed you can incorporate unsloth into your workflow with a very simple manner; instead of loading `AutoModelForCausalLM`, you just need to load a `FastLlamaModel` or `FastMistralModel` as follows:
...
The saved model is compatible with Hugging Face's transformers library. Learn more about unsloth in their [official repository](https://github.com/unslothai/unsloth).

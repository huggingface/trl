...
The [`SFTTrainer`] supports popular dataset formats. This allows you to directly pass the dataset to the trainer without any pre-processing. The following formats are supported:
...
If your dataset is using one of the above formats, you can directly pass it to the trainer without any pre-processing. The [`SFTTrainer`] will then format the dataset for you using the defined format from the models tokenizer with the [apply_chat_template](https://huggingface.co/docs/transformers/main/en/chat_templating#templates-for-chat-models) method. 

...
> [!WARNING]
> This functionality is only available for chat templates that support returning the  assistant tokens mask via the `{% generation %}` keyword. For an example of such an template, check [Qwen/Qwen3-8B/discussions/14](https://huggingface.co/Qwen/Qwen3-8B/discussions/14).

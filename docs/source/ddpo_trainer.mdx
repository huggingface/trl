...
Note that the default implementation has a LoRA implementation path and a nn-LoRA based implementation path. The LoRA flag enabled by default. This can be turned off by passing in the flag to do so. LORA based training is faster to train and the LORA associated model hyperparameters responsible for model convergence aren't as finicky as the non-LORA based training.

...
There is an interface that is provided by this library that are required to be implemented in order to be use the `DDPOTrainer`, which is the main machinery for finetuning. 
There is a default implementation of this interface that you can use out of the box. Assuming the default implementation is sufficient and/or to get thing moving, refer to the training example alongside this readme. 
...
`DDPOConfig` is the config object type. Most of the configuration params have defaults 
...
> [!WARNING]
> When sampling is truncated (`top_p < 1`, `top_k > 0` or `min_p > 0`), the log probabilities vLLM returns are normalised over the tokens that survived the truncation, while the trainer normalises over the full vocabulary. The importance sampling ratio then carries a factor equal to the kept probability mass of each position, which is not a policy change: at `top_p=0.8` on Qwen2.5-1.5B-Instruct the ratio averages 0.90 instead of 1 for an unchanged policy. Set `vllm_importance_sampling_support="sampled"` to have vLLM return the kept token ids of every generated token (`return_sampling_mask`, vLLM >= 0.28, colocate mode) and normalise the trainer's log probabilities over the same set; this needs `top_k > 0`, which vLLM uses to bound the size of the returned set (a large value such as `1024` leaves `top_p` in charge).

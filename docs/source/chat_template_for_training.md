...
If your model supports tool calling, ensure the `{% generation %}` tags also wrap tool call tokens since these are generated by the assistant, but **not** tool responses:
...
### Step 4: Apply the Modified Template

...
| `"alphapo"`                            | The [AlphaPO](https://huggingface.co/papers/2501.03884) method is also implemented in the [`CPOTrainer`]. This is syntactic sugar that automatically sets `loss_type="simpo"` and `cpo_alpha=0.0`. AlphaPO applies an AlphaPO transformation to the rewards in the context of SimPO loss when the `alpha` parameter is non-zero.  |

...
To use this loss as described in the paper, we can set the `loss_type="simpo"`, `cpo_alpha=0.0`, together with `alpha` and `simpo_gamma` to recommended values in the [`CPOConfig`]. Other variants of this method are also possible, such as setting `loss_type="ipo"` and `alpha` to any non-zero value.

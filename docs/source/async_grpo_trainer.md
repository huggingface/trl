...
`vllm_importance_sampling_correction` (on by default) gates the per-token contribution. The default mode, `token_truncate` with clip bounds `[0.5, 3.0]`, clamps the ratio entering the clipped objective to the bounds (before PPO clipping, so the clipped-constant branch cannot be inflated) and leaves in-range tokens untouched: it bounds the unstable upper branch and pathological collapse, without fighting ordinary drift. The bounds deliberately do not touch the top-p support bias itself, which for common `top_p` values sits between 0.9 and 1.0; to lift it, set `vllm_importance_sampling_clip_min` near the surviving probability mass of your sampling configuration. `token_mask` zeroes tokens whose ratio leaves the bounds, and `sequence_mask` zeroes whole packed sequences by the product of their token ratios. Because that product also contains staleness drift and grows with completion length, `sequence_mask` needs far looser bounds here than in [`GRPOTrainer`], where the product covers sampler mismatch alone. The `sampling/importance_sampling_gated_frac` metric reports the fraction of tokens the gate changed. A rising value means the queue is running further off-policy than the clip bounds tolerate, and `max_staleness` or `weight_sync_steps` deserve a look.

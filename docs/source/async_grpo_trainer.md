...
**Checkpoint and resume**: `ignore_data_skip` defaults to `True`; the base Trainer's skip-and-replay loop does not apply to a live rollout queue. Instead, the rollout worker's dataset position is saved to `rollout_state.json` alongside each checkpoint and restored on resume, so the worker fast-forwards to the correct prompt without replaying samples. Streaming datasets (`IterableDataset`) cannot be repositioned; their worker restarts from prompt 0 on resume.

...
Staleness also changes what the loss ratio means. The synchronous trainer recomputes the old log probabilities with its own weights and treats vLLM's logprobs only as the sampler distribution. AsyncGRPO cannot recompute anything, because the weights that generated a sample are gone by the time it is trained on. Its ratio therefore folds policy drift and training-inference mismatch into one number, which the `--logprobs-mode processed_logprobs` server flag makes measurable and the importance-sampling correction bounds. Without the correction, a stale sample with a negative advantage contributes loss that grows linearly with the ratio, and under top-p sampling even an unchanged policy reads a ratio equal to the surviving probability mass instead of 1.0.

`vllm_importance_sampling_correction` (on by default) gates the per-token contribution. The default mode, `token_truncate` with `clip_max=3.0` and no lower bound, clamps the ratio entering the clipped objective (before PPO clipping and in log space, so neither the clipped-constant branch nor float32 `exp` overflow can defeat the bound) and leaves in-range tokens untouched: it bounds the unstable upper branch without fighting ordinary drift. A low ratio needs no bound by default: with a negative advantage it sits in PPO's zero-gradient clipped branch, and with a positive advantage its contribution shrinks with the ratio. The exception is the top-p support bias, where an unchanged policy reads a ratio equal to the surviving probability mass instead of 1.0; to lift it, set `vllm_importance_sampling_clip_min` near that mass. `token_mask` zeroes tokens whose ratio leaves the bounds, and `sequence_mask` zeroes whole packed sequences by the product of their token ratios. Because that product also contains staleness drift and grows with completion length, `sequence_mask` needs far looser bounds here than in [`GRPOTrainer`], where the product covers sampler mismatch alone. The `sampling/importance_sampling_gated_frac` metric reports the fraction of tokens the gate changed. A rising value means the queue is running further off-policy than the clip bounds tolerate, and `max_staleness` or `weight_sync_steps` deserve a look.

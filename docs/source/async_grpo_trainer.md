...
Staleness also changes what the loss ratio means. The synchronous trainer recomputes the old log probabilities with its own weights and treats vLLM's logprobs only as the sampler distribution; AsyncGRPO cannot recompute anything, because the weights that generated a sample are gone by the time it is trained on. Its ratio therefore folds policy drift and training-inference mismatch into one number, which the `--logprobs-mode processed_logprobs` server flag makes measurable — and which the importance-sampling correction bounds. `vllm_importance_sampling_correction` (on by default) gates the per-token loss with the same truncate/mask family as [`GRPOTrainer`]: `*_truncate` modes cap each token's effective gradient weight at the clip bounds, `*_mask` modes zero out tokens or whole sequences whose ratio leaves them. Without the gate, a stale sample with a negative advantage contributes loss that grows linearly with the ratio, and under top-p sampling even an unchanged policy reads a ratio below 1. The `sampling/importance_sampling_gated_frac` metric reports the fraction of tokens the gate touched — a rising value means the queue is running further off-policy than the clip bounds tolerate, and `max_staleness` or `weight_sync_steps` deserve a look.

...
> [!TIP]
> In TRL 0.18 and earlier, packing used a more aggressive method that reduced padding to almost nothing, but had the downside of breaking sequence continuity for a large fraction of the dataset, which hurts performance, as discussed in [Qwen3-Coder-Next Technical Report](https://huggingface.co/papers/2603.00729). To revert to this strategy, use `packing_strategy="wrapped"` in [`SFTConfig`].

...
def add_structures_to_dataset(dataset, max_length_protein: int = 2048, num_proc: int = 192):
    ...
    def process_structure(example):
        ...
        if L > max_length_protein:
            coords = coords[:max_length_protein]
        
        # Store coordinates as list (HF datasets requirement)
        example["structure_coords"] = coords.tolist()
...
async def main(args):
    ...
    _, val_ds, _ = load_cafa5_dataset(
        dataset=args.cafa5_hf_repo,
        dataset_name=args.cafa5_config,
        ...
        interpro_dataset_name=args.interpro_config,
        include_protein_function_summary=False,
        debug=False,
    )

    n = min(args.max_samples, len(val_ds))
...
if __name__ == "__main__":
    ...
    p.add_argument("--cafa5_hf_repo", type=str, default="wanglab/cafa5")
    p.add_argument("--cafa5_config", type=str, default="cafa5_reasoning")
    p.add_argument("--dataset_cache_dir", type=str, default="/large_storage/goodarzilab/parsaidp/BioReason2/data/")
    p.add_argument("--structure_dir", type=str, default="/large_storage/goodarzilab/bioreason/data/sequences/")
    p.add_argument("--interpro_config", type=str, default="interpro_metadata")

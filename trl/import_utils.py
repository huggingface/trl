...
def is_vllm_available() -> bool:
    if _vllm_available and version.parse(_vllm_version) != version.parse("0.10.2"):
        warnings.warn(
            "TRL currently only supports vLLM version `0.10.2`. You have version "
            f"{_vllm_version} installed. We recommend to install this version to avoid compatibility issues.",
            UserWarning,
        )

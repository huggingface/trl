...
class GRPOTrainer(Trainer):
    ...
    def _generate_and_score_completions(
        ...
        if self.max_prompt_length is not None:
            ...
            # collapse them back into a single token string to match the original template in case it originally
            # applies it. Otherwise, it assumes that the template uses only vision_start_token_id to indicate images
            # (e.g. Gemma 3) and removes all image_token instances and vision_end_token_id as well.

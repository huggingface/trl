...
class GRPOTrainer(BaseTrainer):
    ...
    def __init__(
        ...
        if self.args.log_completions and self.args.log_completions_hub_repo is not None:
            from huggingface_hub import DatasetCard, create_repo
...
The completions are stored in parquet files, and each file contains the completions for a single step of training
(depending on the `logging_steps` argument).
...
Additionally, the individual reward scores are also logged using the column naming convention
`reward_<reward_function_name>`. This can be used to better understand how the reward functions are working and how
they interact with each other.

Having this data stored as a simple parquet file makes it easy to load and analyze using the Datasets Viewer, Polars,
Pandas, etc.
...
dataset = datasets.load_dataset({self.args.log_completions_hub_repo})
```
...
"""
    ...
    def log(self, logs: dict[str, float], start_time: float | None = None) -> None:
        ...
        if self.accelerator.is_main_process and self.log_completions:
            ...
            df_base.to_parquet(
                os.path.join(
                    f"{self.args.output_dir}/completion_logs", f"completions_{self.state.global_step}.parquet"
                )
            )

...
class GRPOTrainer(Trainer):
    ...
    def compute_liger_loss(self, unwrapped_model, inputs):
        ...
        for i in range(0, inputs["advantages"].size(0), self.num_generations_chunks):
            last_hidden_state = self._get_last_hidden_state(unwrapped_model, input_ids[i:i+self.num_generations_chunks], attention_mask[i:i+self.num_generations_chunks], logits_to_keep)
    ...
    def _compute_loss(self, model, inputs):
        ...
        # ---------- containers to accumulate per-chunk stats ----------
        mean_kls,     clip_ratios      = [], []
        low_clips,    high_clips       = [], []
        region_clips                   = []
        
        # Compute the loss
        for i in range(0, inputs["advantages"].size(0), self.num_generations_chunks):
            if self.token_entropy_percentile_threshold > 0.0:
                logps_and_entropies = self._get_per_token_logps_and_entropies(model, 
            ...
            region_clip     = is_low_clipped | is_high_clipped

            low_clips.append((is_low_clipped  * completion_mask_slice).sum()
                             / completion_mask_slice.sum())
            high_clips.append((is_high_clipped * completion_mask_slice).sum()
                              / completion_mask_slice.sum())
            region_clips.append((region_clip   * completion_mask_slice).sum()
                                / completion_mask_slice.sum())
        ...
        self._metrics[mode]["clip_ratio/low_mean"].append(_gather_and_reduce(low_clips,   "mean"))
        self._metrics[mode]["clip_ratio/low_min"].append(_gather_and_reduce(low_clips,   "min"))
        self._metrics[mode]["clip_ratio/high_mean"].append(_gather_and_reduce(high_clips,  "mean"))
        self._metrics[mode]["clip_ratio/high_max"].append(_gather_and_reduce(high_clips,  "max"))
        self._metrics[mode]["clip_ratio/region_mean"].append(_gather_and_reduce(region_clips,"mean"))

    ...
    def train(self):
        ...
        for update in range(1, args.num_total_batches + 1):
            ...
            with torch.no_grad():
                ...
                #### Normalizing the scores by the query
                num_original_queries = queries.shape[0] // args.sampling_group_size
                normalized_scores = []

                for i in range(num_original_queries):
                    group_scores = scores[i * args.sampling_group_size : (i + 1) * args.sampling_group_size]
                    mean = group_scores.mean()
                    std = group_scores.std()
                    normalized_group_scores = (group_scores - mean) / (std + 1e-8)  # add small epsilon to avoid division by zero
                    normalized_scores.append(normalized_group_scores)

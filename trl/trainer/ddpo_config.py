...
class DDPOConfig(object):
    ...
    allow_tf32: Optional[bool] = field(
        default=True, metadata={"help": "Allow tf32 on Ampere GPUs."}
    )
    resume_from: Optional[str] = field(
        default="", metadata={"help": "Resume training from a checkpoint."}
    )
    use_lora: Optional[bool] = field(
        default=True, metadata={"help": "Whether or not to use LoRA."}
    )
    pretrained_model: Optional[str] = field(
        default="runwayml/stable-diffusion-v1-5",
        metadata={"help": "Base model to load."},
    )
    pretrained_revision: Optional[str] = field(
        default="main", metadata={"help": "Revision of the model to load."}
    )
    sample_num_steps: Optional[int] = field(
        default=50, metadata={"help": "Number of sampler inference steps."}
    )
    sample_eta: Optional[float] = field(
        default=1.0, metadata={"help": "Eta parameter for the DDIM sampler."}
    )
    sample_guidance_scale: Optional[float] = field(
        default=5.0, metadata={"help": "Classifier-free guidance weight."}
    )
    ...
    train_batch_size: Optional[int] = field(
        default=1, metadata={"help": "Batch size (per GPU!) to use for training."}
    )
    ...
    train_cfg: Optional[bool] = field(
        default=True,
        metadata={
            "help": "Whether or not to use classifier-free guidance during training."
        },
    )
    train_adv_clip_max: Optional[float] = field(
        default=5, metadata={"help": "Clip advantages to the range."}
    )
    train_clip_range: Optional[float] = field(
    ...
    per_prompt_stat_tracking_buffer_size: Optional[int] = field(
        default=16,
        metadata={
            "help": "Number of reward values to store in the buffer for each prompt."
        },
    )
    per_prompt_stat_tracking_min_count: Optional[int] = field(
        default=16,
        metadata={
            "help": "The minimum number of reward values to store in the buffer."
        },
    )

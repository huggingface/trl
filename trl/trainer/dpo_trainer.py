...
def _tokenize(features: Dict[str, List], tokenizer, processor=None, model=None, args=None) -> Dict[str, List]:
    """Tokenize a batch of rows from a DPO specific dataset."""
    batch_size = len(features["prompt"])
    batch = defaultdict(list)

    for i in range(batch_size):
        feature = {k: v[i] for k, v in features.items()}
        single_result = _tokenize_row(feature, tokenizer, processor, model, args)
...
def _tokenize_row(feature, tokenizer, processor=None, model=None, args=None) -> Dict:
    """Tokenize a row from a DPO specific dataset.
    ...
    if not args.is_encoder_decoder:
        ...
        if processor is not None:
            if "add_special_tokens" in inspect.signature(processor).parameters:
                ...
                processor_kwargs = {}
            prompt_tokens = processor(prompt, images=images, **processor_kwargs)
        ...
        chosen_tokens = _build_tokenized_answer(prompt, chosen, images, processor=processor, tokenizer=tokenizer)

        if not isinstance(rejected, str):
            raise ValueError(f"rejected should be an str but got {type(rejected)}")
        rejected_tokens = _build_tokenized_answer(prompt, rejected, images, processor=processor, tokenizer=tokenizer)
...
def _build_tokenized_answer(
    prompt: str, answer: str, images: Optional[List[Any]] = None, processor=None, tokenizer=None
) -> Dict[str, Any]:
    """Build tokenized answer, handling vision models and different tokenizers."""
    """
    ...
    if processor is not None:
        ...
        if "add_special_tokens" in inspect.signature(processor).parameters:
            ...
            processor_kwargs = {}
        full_tokenized = processor(prompt + answer, images=images, **processor_kwargs)
        ...
        prompt_input_ids = processor(prompt, images=images, **processor_kwargs)["input_ids"][0]

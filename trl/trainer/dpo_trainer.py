...
def build_tokenized_answer(tokenizer, processor, tokenize_row_config: TokenizeRowConfig, prompt, answer, images=None):
    ...
    """
    if tokenize_row_config.is_vision_model:
...
def tokenize_row(
    ...
    if not tokenize_row_config.is_encoder_decoder:
        ...
        for answer_tokens in [chosen_tokens, rejected_tokens, prompt_tokens]:
            if len(answer_tokens["prompt_input_ids"]) + longer_response_length > tokenize_row_config.max_length:
                if tokenize_row_config.truncation_mode == "keep_start":
                    for k in ["prompt_input_ids", "prompt_attention_mask"]:
                        answer_tokens[k] = answer_tokens[k][: tokenize_row_config.max_prompt_length]
                elif tokenize_row_config.truncation_mode == "keep_end":
                    for k in ["prompt_input_ids", "prompt_attention_mask"]:
                        answer_tokens[k] = answer_tokens[k][-tokenize_row_config.max_prompt_length :]
                else:
                    raise ValueError(f"Unknown truncation mode: {tokenize_row_config.truncation_mode}")
        ...
        chosen_sequence_tokens["labels"][: len(chosen_tokens["prompt_input_ids"])] = [
            tokenize_row_config.label_pad_token_id
        ...
        chosen_tokens = tokenizer(
            chosen, truncation=True, max_length=tokenize_row_config.max_target_length, add_special_tokens=True
        )
        rejected_tokens = tokenizer(
            rejected, truncation=True, max_length=tokenize_row_config.max_target_length, add_special_tokens=True
        )
        prompt_tokens = tokenizer(
            prompt, truncation=True, max_length=tokenize_row_config.max_prompt_length, add_special_tokens=True
        )

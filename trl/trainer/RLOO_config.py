...
class RLOOConfig(TrainingArguments):
    ...
    Only the parameters specific to RLOO training are listed here. For details on other parameters, refer to the
    ...
    Parameters:
        ...
        #! beta same as kl_coef in original RLOO
    ...
    def __post_init__(self):
        ...
        #! We can have a single generation per prompt for RLOO
        # if self.num_generations < 2:
        #     raise ValueError(
        #         "GRPO requires at least 2 generations per prompt to calculate the advantages. You provided "
        #         f"{self.num_generations}, which is less than the minimum required."
        #     )
        # ! range changed to start from 1
        possible_values = [
            n_gen for n_gen in range(1, self.generation_batch_size + 1) if (self.generation_batch_size) % n_gen == 0
        ]

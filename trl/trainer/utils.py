...
def chunked_cross_entropy_loss(
    ...
    The full `lm_head` projection is never materialized. Positions where `labels == ignore_index`
    are dropped before the matmul, and the remaining tokens are processed in chunks of
    `chunk_size`. Each chunk's `[chunk_size, vocab_size]` logits tensor is kept alive only
    during its own forward/backward pass via gradient checkpointing, so peak activation memory
    is `chunk_size * vocab_size` instead of `N_valid * vocab_size`.
    ...
    Args:
        ...
        labels (`torch.Tensor`):
            Labels of shape `(B, S)`. Positions equal to `ignore_index` are excluded from both
            the `lm_head` matmul and the loss.
        chunk_size (`int`):
        ...
        num_items_in_batch (`torch.Tensor`, `int` or `None`, *optional*):
            Total number of valid tokens across the global batch, as plumbed by
            [`~transformers.Trainer`]. When provided, the loss is reduced as
            `sum / num_items_in_batch`, matching the gradient-accumulation-correct behavior of
            HF's default cross-entropy. When `None`, reduction is `mean` over local valid tokens.
        ignore_index (`int`, *optional*, defaults to `-100`):
            Label value marking positions to skip.
        logit_scale (`float`, *optional*, defaults to `1.0`):
            Multiplier applied to each chunk's logits before the cross-entropy, matching the
            `logit_scale` behavior of Cohere-style models.
        final_logit_softcapping (`float`, *optional*):
            If set, applies `softcap * tanh(logits / softcap)` to each chunk's logits before the
            cross-entropy, matching the `final_logit_softcapping` behavior of Gemma-style models.
            Applied after `logit_scale`.

    Returns:
        `tuple[torch.Tensor, torch.Tensor, torch.Tensor]`: scalar loss, scalar token accuracy, and
        scalar mean Shannon entropy (in nats) — all taken over non-ignored tokens in the local
        batch. When there are no valid tokens, all three are `0.0`.
    """
    ...
    def _chunk(h, w, l):
        ...
        chunk_loss = F.nll_loss(log_p, l, reduction="sum")
        chunk_correct = (logits.argmax(dim=-1) == l).sum().float()
    ...
    for start in range(0, n_valid, chunk_size):
        ...
        chunk_loss, chunk_correct, chunk_entropy = torch.utils.checkpoint.checkpoint(
            _chunk, h_chunk, lm_head_weight, l_chunk, use_reentrant=False
        )
...
def patch_chunked_ce_lm_head(model: torch.nn.Module, chunk_size: int) -> None:
    """
    Patch a causal LM so its `forward` computes the language modeling loss via
    [`chunked_cross_entropy_loss`] when `labels` are provided.

    The patched forward calls the base decoder directly (`model.model`) to obtain hidden states,
    skips the `lm_head` matmul on positions with `labels == -100`, and computes the cross-entropy
    in chunks of `chunk_size` valid tokens. It returns a
    [`~transformers.modeling_outputs.CausalLMOutputWithPast`] with `loss` set, `logits` set to
    `None`, and a dynamic `token_accuracy` attribute holding the mean next-token accuracy over
    non-ignored tokens. When `labels` is `None`, the original `lm_head` path is used so
    generation / evaluation without labels keeps working.

    Not supported: VLM / multimodal models whose forward injects visual tokens outside the base
    decoder, and PEFT-wrapped models.
    """
    ...
    def _chunked_ce_forward(
        ...
        **kwargs,
        ...
        loss, token_accuracy, entropy = chunked_cross_entropy_loss(

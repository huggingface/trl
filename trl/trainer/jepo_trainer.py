...
.
    ...
    see the jepo_training notebook for a complete example of how to use the JEPOTrainer.
    ```
    ...
    Args:
        ...
        cot_func (`CoTFunc`):
            CoT function used to extact chain-of-thought (CoT) from the generated completion and generate CoT-only and CoT+Answer formatted text.
                
        args ([`JEPOConfig`], *optional*):
    ...
    def __init__(
        ...
        self.supervised_loss_weight = args.supervised_loss_weight
        # Training arguments
    ...
    def _generate_single_turn(self, prompts: list[any], images: Optional[list]):
        device = self.accelerator.device
    ...
    def _compute_jepo_advantages(self, jepo_rewards, jepo_applicable_mask, num_generations, device="cpu"):  
        """  
        Compute JEPO advantages for a rewards tensor by groups.  
        
        Args:  
            jepo_rewards (Tensor): A 1D tensor of rewards.  
            jepo_applicable_mask (Tensor): A boolean 1D tensor of the same length as jepo_rewards.  
                True indicates that the sample is applicable.  
            num_generations (int): Number of samples (generations) per group.  
            device (str): Device specifier ("cpu" or "cuda").  
        
        Returns:  
            Tensor: A tensor of advantages, with the same shape as jepo_rewards.  
        """  
        jepo_advantages = torch.zeros_like(jepo_rewards)  
        ...
        for i in range(num_groups):  
            ...
            # Process only if at least one sample in the group is applicable.  
            if jepo_applicable_mask[group_slice].any():  
                # Count the applicable (True) samples and convert to float.  
                applicable_count = jepo_applicable_mask[group_slice].sum().float()  
                # Sum rewards for the group.  
                group_reward_sum = jepo_rewards[group_slice][jepo_applicable_mask[group_slice]].sum()  
                # Compute the mean over the applicable samples.  
                mean_reward = group_reward_sum / applicable_count  
    
                # List to store computed advantages for later normalization.  
                group_advantages_list = []  
                
                for j in range(start_idx, end_idx):  
                    if jepo_applicable_mask[j]:  
                        if applicable_count > 1:  
                            # Compute a leave-one-out "variance" (mean of the other rewards).  
                            variance = (group_reward_sum - jepo_rewards[j]) / (applicable_count - 1)  
                            # JEPO advantage encourages samples that have high and consistent rewards.  
                        ...
                        jepo_advantages[j] = advantage  
                        group_advantages_list.append(advantage)  
                
                # Normalize the advantages within the group by their standard deviation.  
                # If there is only one applicable sample, we use a standard deviation of 1.  
                if len(group_advantages_list) > 1:  
                    group_std = torch.std(torch.stack(group_advantages_list))  
                else:  
                    group_std = torch.tensor(1.0, device=device)  
                #print(f"Group {i}: mean_reward={mean_reward:.4f}, std={group_std:.4f}, applicable_count={applicable_count.item()} group_reward_sum={group_reward_sum:.4f}")      
                for j in range(start_idx, end_idx):  
                    if jepo_applicable_mask[j]:  
    ...
    def _generate_and_score_completions(
        ...
        # compute jepo advantages which is based on p(anwer|question, CoT), CoT is from the sampled completions_text
        ...
        with torch.no_grad():
            ...
            #calclate the CtO logits and entropy, which is used to calculate the JEPO rewards. The JEPO reward is calculated as the log probability of the completion given the prompt and the sampled CoT, minus the log probability of the completion given only the prompt (i.e., without the CoT). The intuition is that if the sampled CoT is helpful for generating the completion, then the log probability of the completion given the prompt and CoT should be higher than the log probability of the completion given only the prompt, resulting in a positive JEPO reward.
            #synthesize the completion with sampled CoT from completions_text and answer

            def synthesize_completion(prompt, completion, answer):
                correct_format, cot_text, cot_answer_text = self.cot_func(completion[0]['content'], answer)
                
                #print(f"prompt: {prompt}, completion: {completion}, answer: {answer}, correct_format: {correct_format}, cot_text: {cot_text}, cot_answer_text: {cot_answer_text}")
                return (correct_format, maybe_apply_chat_template({"prompt": prompt, 'completion': [{"role": "assistant", "content": cot_text}]}, self.processing_class)['completion'],
                        maybe_apply_chat_template({"prompt": prompt, 'completion': [{"role": "assistant", "content": cot_answer_text}]}, self.processing_class)['completion'])
            
            cot_texts = []
            cot_answer_texts = []
            jepo_applicable_mask = torch.zeros(len(prompts), dtype=torch.bool, device=device)
            for i, (prompt, completion, answer) in enumerate(zip(prompts, completions, [x['answer'] for x in inputs])):
                correct_format, cot_text, cot_answer_text = synthesize_completion(prompt, completion, answer)
                cot_texts.append(cot_text)
                cot_answer_texts.append(cot_answer_text)

                if correct_format:
                    jepo_applicable_mask[i] = True  # mark this sample as applicable for JEPO reward

            cot_text_tokens = self.processing_class(
                text=cot_texts, 
                padding=True, 
                padding_side="right",
                ...
                truncation=True,
                add_special_tokens=False)  
            cot_text_tokens = super()._prepare_inputs(cot_text_tokens)

            cot_answer_text_tokens = self.processing_class(
                text=cot_answer_texts, 
                padding=True, 
                padding_side="right",
                max_length=self.max_completion_length,
                return_tensors="pt", 
                truncation=True,
                add_special_tokens=False)  
            cot_answer_text_tokens = super()._prepare_inputs(cot_answer_text_tokens)


            fabricated_completions_ids, fabricated_completions_mask = cot_answer_text_tokens["input_ids"], cot_answer_text_tokens["attention_mask"]

            input_ids = torch.cat([prompt_ids, fabricated_completions_ids], dim=1)
            attention_mask = torch.cat([prompt_mask, fabricated_completions_mask], dim=1)


            # we only need to compute the logits for the answer (A| Q,C)
            logits_to_keep = fabricated_completions_ids.size(1)  # we only need to compute the logits for the fabricated completion tokens, which is the CoT answer part

            #print(f"fabricated_completions_ids shape: {fabricated_completions_ids.shape}, fabricated_completions_mask shape: {fabricated_completions_mask.shape},logits_to_keep: {logits_to_keep}")
            ...
            padded_cot_attention_mask = cot_text_tokens["attention_mask"]
            if cot_text_tokens["attention_mask"].shape[1] != fabricated_completions_mask.shape[1]:
                # if the cot_text_tokens attention mask is shorter than the fabricated_completions_mask, we need to pad it to the same length
                padding_length = fabricated_completions_mask.shape[1] - cot_text_tokens["attention_mask"].shape[1]
                padded_cot_attention_mask = torch.cat([cot_text_tokens["attention_mask"], torch.zeros((cot_text_tokens["attention_mask"].shape[0], padding_length), dtype=torch.long, device=device)], dim=1)
            # there is EOT_TOKEN at the end of the cot_text and cot_answer_text, using answer_mask = fabricated_completions_mask - padded_cot_attention_mask decode shows "answer>\n0\n</answer>", "<" is missing because of the EOT_TOKEN
            ...
            #IMPORTANT: be careful of the exp need to be done first before sum, otherwise it would be sum of log probabilities which is not what we want (since prob =0 still has the advantage of 1)
            jepo_rewards = (torch.exp(per_token_logps) * answer_mask).sum(dim=1)  # zero out the log probabilities for the non-answer tokens
            print(f"jepo_rewards: {jepo_rewards}")
            #print the prob of the answer:
        ...
        print(f"format advantages shape: {advantages.shape}, {advantages} , jepo_advantages shape: {jepo_advantages.shape}, {jepo_advantages}")
        #sum formatted rewards and jepo rewards to get the final rewards, then calculate the final advantages
        advantages = advantages + jepo_advantages
        ...
        output = {
            ...
            "fabricated_completions_ids": fabricated_completions_ids,
            "fabricated_completions_mask": fabricated_completions_mask,
    ...
    def _compute_loss(self, model, inputs):
        ...
        fabricated_completions_ids, fabricated_completions_mask = inputs["fabricated_completions_ids"], inputs["fabricated_completions_mask"]
        answer_mask = inputs["answer_mask"]
        input_ids = torch.cat([prompt_ids, fabricated_completions_ids], dim=1)
        attention_mask = torch.cat([prompt_mask, fabricated_completions_mask], dim=1)

        logits_to_keep = fabricated_completions_ids.size(1)  # we only need to compute the logits for the fabricated completion tokens
        ...
        if self.supervised_loss_weight > 0.0:
            # Add a supervised loss to encourage the model to generate the reference answer. We use the answer_mask to calculate the supervised loss only on the answer part, which is p(A|Q,C). The intuition is that we want to encourage the model to generate answers that are likely under the reference model, which can help stabilize training and prevent divergence.
            answer_token_logps = torch.exp(per_token_logps ) * answer_mask  
            if self.loss_type == 'unnorm_jepo':
                supervised_loss = answer_token_logps.sum(dim=1)  # sum over the answer tokens to get the total log probability of the answer given the question and CoT, which is p(A|Q,C), we will use it as a supervised loss to encourage the model to generate answers that are likely under the reference model, which can help stabilize training and prevent divergence.
            else:
        ...
        completion_token_count = attention_mask.sum().clamp(min=1.0)

        def masked_batch_mean(x):
            if x.shape[1] == 1:  # when importance_sampling_level == "sequence"
                return x.mean()
            else:
                return (x * fabricated_completions_mask).sum() / completion_token_count

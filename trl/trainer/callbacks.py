    ...
    def on_train_end(self, args: TrainingArguments, state: TrainerState, control: TrainerControl, **kwargs):
        ...
        with distributed_state.split_between_processes(self.messages, apply_padding=True) as messages:
            for message in messages:
                print(f"Generating text for message: {message}")
                tokenized_message = self.tokenizer.apply_chat_template([message], tokenize=True, add_generation_prompt=True, return_tensors="pt").to(self.model.device)
                output = self.model.generate(tokenized_message, max_new_tokens=32, do_sample=True, temperature=0.7)
        ...
        if len(completions) != len(self.messages):
            # Drop final sample as it's a duplicate

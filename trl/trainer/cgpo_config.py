...
class CGPOConfig(TrainingArguments):
    ...
    Parameters:
        k (`int`, *optional*, defaults to `4`):
            The number of responses generated by the policy for each prompt at each iteration.
        rlhf_optimizer (`Literal["crraft", "codpo", "crpg"]`, *optional*, defaults to `crraft`):
            The RLHF optimizer to update the policy during CGPO training. Possible options:
            - CRRAFT (Calibrated-Regularized Reward Ranking Fine-tuning)
            - CODPO (Constrained Online Direct Preference Optimization)
            - CRPG (Calibrated-Regularized Policy Gradient)
        kl_threshold (`float`, *optional*, defaults to `None`):
            Maximum allowable KL-divergence during policy updates, used to prevent significant deviation from the reference model
        beta (`float`, *optional*, defaults to `0.1`):
            Used when `rlhf_optimizer` is set to `codpo`.
            It controls the deviation from the reference model. Higher beta means less deviation from the reference model.
        ...
        local_policy_optimization_batch_size (`int`, *optional*, defaults to `None`):
            The size of the local mini-batch used during policy optimization.
        max_new_tokens (`int`, *optional*, defaults to `64`):
            Maximum number of tokens to generate per completion.
        max_length (`int`, *optional*, defaults to `None`):
            The maximum sequence length for the prompt and the baseline completions.
    ...
    local_policy_optimization_batch_size: int = None

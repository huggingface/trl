    ...
    def on_log(self, args: TrainingArguments, state: TrainerState, control: TrainerControl,**kwargs):
        if args.logging_strategy == 'steps':
            ...
            if 'loss' in state.log_history[-1]:
                ...
                mlflow.log_metric("accuracy", state.log_history[-1]["accuracy"] , step=state.global_step)
                print(f'loss: {state.log_history[-1]["loss"]}, lr: {state.log_history[-1]["learning_rate"]}, throughput: {throughput}, step: {state.global_step}, accuracy: {state.log_history[-1]["accuracy"]}')       

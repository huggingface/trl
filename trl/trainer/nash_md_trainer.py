    ...
    def train(self):
        ...
        for update in range(1, args.num_updates + 1):
            ...
            with torch.no_grad():
                ...
                for i in range(0, queries.shape[0], args.local_rollout_forward_batch_size):
                    ...
                    if args.non_eos_penalty:
                        model_resp_contains_eos = torch.any(postprocessed_query_response == tokenizer.eos_token_id, dim=-1)
                        model_reward = torch.where(model_resp_contains_eos, model_reward, args.penalty_reward_value)
                        ref_resp_contains_eos = torch.any(postprocessed_mixture_query_response == tokenizer.eos_token_id, dim=-1)
                ...
                for mini_batch_start in range(
                    ...
                    for micro_batch_start in range(
                        ...
                        with accelerator.accumulate(model):
                            ...
                            preference_losses = - model_logprobs_sum * (preference_model_vs_mixture - 0.5)    # 0.5 is a control variate

                            ## KL loss
                            raw_kl_model_vs_ref = torch.sum( torch.exp(model_logprobs) * (model_logprobs - ref_logprobs), dim=2)
                            ...
                            with torch.no_grad():
                                ...
                                preference_loss_stats[epoch_idx, minibatch_idx, gradient_accumulation_idx] = preference_losses.mean()
            ...
            with torch.no_grad():
                ...
                metrics["objective/preference"] = self.accelerator.gather(preference_model_vs_mixture_stats).mean().item()

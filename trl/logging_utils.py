...
def build_html_table(table: dict[str, list[Any]]):
    ...
    col_width = 100 // len(keys) if keys else 100
    columns = ''.join(
        f'<th style="border: 1px solid black; padding: 6px; width:{col_width}%">{html.escape(str(key).capitalize())}</th>'
        for key in keys
    )

    html_str = MODAL_STYLE + MODAL_SCRIPT + f"""
    ...
    for row in zip(*[table[k] for k in keys]):
        ...
        for value in row:
            cell_html = (
                ...
                f"<pre style='margin:0; height:100%;'>{html.escape(str(value))}</pre>"
                "</div>"
                "</div>"
            )
            html_str += f'<td style="border: 1px solid black; padding: 6px; vertical-align: top;">{cell_html}</td>'
        html_str += "</tr>"

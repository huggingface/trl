...
Construct **one** ``HarborSpec`` and read three properties off it — ``.train_dataset``,
``.environment_factory``, ``.reward_funcs`` — each plugging into the matching ``GRPOTrainer`` kwarg:
...
A Harbor *task* is a directory (``instruction.md`` + ``task.toml`` + ``environment/`` + ``tests/``);
the dataset is a tree of them. The ``environment_factory`` env runs Harbor in-process (see ``_env.py``),
so ``harbor`` must be installed in the same interpreter (``pip install trl[harbor]``, Python >= 3.12).
The **base agent** (harness/tool surface) is selected by ``agent=`` — ``"bash"`` today, or a custom
``HarborEnv`` subclass.
"""
...
def _resolve_agent(agent: str | type[HarborEnv]) -> type[HarborEnv]:
    ...
    Accepts a `HarborEnv` subclass, a built-in name (`"bash"`), a module import path
    (`"pkg.module:Class"`), or a file path (`"path/to/file.py:Class"`).
    """
    if isinstance(agent, type):
        ...
        target, _, cls_name = agent.partition(":")
...
class HarborSpec:
    ...
    Args:
        dataset (`str`):
            ...
            `"AdithyaSK/data_agent_rl_environment_train"`), or a local path to a directory containing
            a `tasks/` subtree. Each task is a dir with `instruction.md` / `task.toml` / `environment/`
            / `tests/`.
        agent (`str` or `type`, *optional*, defaults to `"bash"`):
            The base agent / harness — i.e. the tool surface the env exposes. One of: a built-in name
            (`"bash"`), an import path `"package.module:ClassName"`, a file path
            `"path/to/file.py:ClassName"`, or a [`~trl.experimental.harbor.HarborEnv`] subclass directly.
        environment_type (`str`, *optional*, defaults to `"docker"`):
            Harbor sandbox backend, passed through to Harbor (whatever it supports — `"docker"`,
            `"e2b"`, `"daytona"`, `"gke"`, `"modal"`, `"runloop"`, ...). Not validated here; Harbor
            validates. `"docker"` is Harbor's own default; pick `"e2b"` to offload sandboxing to the cloud.
    ...
    def train_dataset(self):
        ...
        Columns: `prompt` (empty user message — TRL appends the env's instruction from `reset`),
        `task_dir` (passed to `reset`), `task_index`, and per-task metadata when `include_metadata`.
        """
        ...
        rows: dict[str, list[Any]] = {
            ...
            "task_index": list(range(len(dirs))),
        }
    ...
    def environment_factory(self) -> Callable[[], HarborEnv]:
        ...
        def _make() -> HarborEnv:
            return env_cls(environment_type=environment_type)

...
class SDPOTrainer(_BaseTrainer):
    ...
    def __init__(
        ...
        if args.use_fused_linear_loss:
            if args.distillation_weight != 1.0:
                raise ValueError(
                    "`use_fused_linear_loss` only supports pure self-distillation with `distillation_weight=1.0`, got "
            ...
            if args.loss_type != "bnpo":
                logger.warning(
                    "The fused linear loss reduces with a token-level mean (equivalent to `loss_type='bnpo'`); the "
                    f"configured `loss_type={args.loss_type!r}` is ignored on the fused path."
                )
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        if self.args.distillation_weight == 1.0:
            if self.use_teacher_server:
                ...
                return self._compute_fused_linear_loss(model, inputs) / accumulation_scale
            else:

        ...
        if self.args.distillation_weight == 1.0:
            if self.use_teacher_server:
                ...
                if nonfinite.any():
                    ...
                    # gets its own message. `warning_once` caches on the message, so this also stops a repeated
                    # evaluation warning from consuming the slot a later training step needs.
                    if mode == "train":
                        logger.warning_once(

    ...
    def _prepare_dataset_with_original_text(
        ...
        with global_then_local_main_first():
            ...
            def tokenize_with_original_text(
                example, processing_class, dataset_text_field, max_length, tool_chat_template, use_extended_uld
            ):
                ...
                if emit_tool_mask:
                    if use_extended_uld:
                        token_spans = full_offs
                    else:
                        # Positional ULD tokenizes with char offsets, while `tool_ranges` are byte spans; convert.
                        char_to_byte = list(accumulate((len(c.encode("utf-8")) for c in full_text), initial=0))
                        token_spans = [(char_to_byte[s], char_to_byte[e]) for s, e in encoding["offset_mapping"]]
                    tool_mask = [

...
class KLPOTrainer(GRPOTrainer):
    ...
    the current model, extracts the estimator's records from that distribution, and keeps them fixed while the batch is
    reused. The sampler's action log-probabilities follow GRPO's `old_per_token_logps` convention: when the batch is
    guaranteed on-policy they are not recomputed, and `ell` is exactly zero. Following the report, no vLLM
    importance-sampling multiplier is applied to the loss, and the group-relative `advantages` computed by
    [`GRPOTrainer`] are ignored in favor of the raw terminal reward.
    ...
    def _record_sampler_data(
        ...
        Depending on `kl_estimator`, records:
        ...
        The `"binary"` estimator needs only the sampled-action log-probabilities, which GRPO's own
        `old_per_token_logps` path already provides, so this method is not called for it.
        """
        mode = "train" if self.model.training else "eval"
        batch_size = self.args.per_device_train_batch_size if mode == "train" else self.args.per_device_eval_batch_size
        all_record_ids, all_record_logps, all_full_logps = [], [], []
    ...
    def _generate_and_score_completions(
        ...
        # extra records. `old_per_token_logps` (the sampler's action log-probabilities, used by the feedback
        # coefficient) is inherited from GRPO: when it is absent the batch is guaranteed on-policy, and _compute_loss
        # falls back to per_token_logps.detach() so that ell is exactly zero (recomputing it in a separate forward
        # would perturb ell with dtype and dropout noise).
    ...
    def _compute_loss(self, model, inputs):
        ...
        # Sampler action log-probabilities. When absent (see _generate_and_score_completions in GRPO), the batch is
        # guaranteed on-policy: fall back to per_token_logps.detach() so that ell is exactly zero rather than
        # perturbed by recomputation noise from a separate forward pass.

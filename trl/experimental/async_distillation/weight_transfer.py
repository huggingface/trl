...
class WeightTransferClient:
    ...
    def init_weight_transfer(self) -> None:
        ...
        # Trainer and server precisions should agree: the precision gap between the two makes the sampled completions
        # off-policy for the student being trained
        ...
        if vllm_dtype != train_dtype:
            logger.warning(
                ...
                f"`AsyncDistillationConfig` to '{vllm_dtype}', or start the server with `--dtype {train_dtype}`."
            )

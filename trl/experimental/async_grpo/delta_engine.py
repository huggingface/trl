...
class DeltaWeightTransferUpdateInfo(WeightTransferUpdateInfo):
    ...
    Names and per-param nnz are read from the downloaded file, so ``num_updates_list`` is not
    required here (we override the base validation that would otherwise demand it for sparse).
    """
...
class DeltaWeightTransferEngine(WeightTransferEngine[DeltaWeightTransferInitInfo, DeltaWeightTransferUpdateInfo]):
    ...
    def upload(
        ...
        Returns the :class:`PatchMetadata` (also written to the safetensors header), or ``None``
...
def encode_patch(
    ...
    - ``mask`` provided: GPU sparse-extract; store encoded indices as ``{name}.idx`` and values
      as ``{name}.val``. ``encoding`` is ``"raw"`` (int32) or ``"gap_delta"`` (uint16 gap bytes,
      uint32 fallback per param).
    """
    encoding = Encoding(encoding)
...
def iter_sparse_patches(f) -> Iterator[tuple[str, torch.Tensor, torch.Tensor]]:
    ...
    Flat, self-describing format: param names are recovered from the ``{name}.val`` tensor keys and
    the index encoding is a single global header field (the gap-delta width is carried by the index
    tensor's own dtype). ``f`` is a ``safetensors.safe_open`` handle.
    """

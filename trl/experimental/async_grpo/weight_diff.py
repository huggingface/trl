...
class AdamWInversionChangeDetector:
    ...
    then compare it to the live weights at **bf16 resolution** (``live.to(bfloat16) != theta_old.to(bfloat16)``) — an
    element is "changed" iff its bf16 value differs. Persistent extra storage is **zero**, the reconstruction is
    transient and thrown away.

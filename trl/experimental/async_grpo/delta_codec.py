...
The change mask, ``nonzero``, and value gather all run on the device, so only the sparse
payload — and, for the disk transport, its compressed index form — crosses PCIe. This
replaces the dense ``tensor.to(bf16).cpu()`` + CPU ``nonzero``/gather in the v1 upload path,
which copied the full dense tensor (~100%) to host just to keep ~1-3% of it.
...
def extract_sparse_batched(
    ...
    ``nonzero`` forces a device→host sync (its output size is data-dependent), so the per-param
    loop costs one sync per param. This concatenates all flattened masks, runs **one** ``nonzero``
    over the whole set, then splits the global positions back per param with ``searchsorted`` on the
    cumulative sizes — collapsing ~N syncs into 2 (the ``nonzero`` and one boundary D2H). Indices are
    returned local to each param's flat space (ready for that param's ``index_copy_``).

    All tensors must be on the same device. (Transient cost: a concatenated full-size bool mask —
    fine for the model sizes here; very large models should shard, see the multi-file TODO.)
    ...
    Returns:
        `list[tuple[str, torch.Tensor, torch.Tensor]]`: ``(name, int32 local indices, values)`` per
        input param, in input order.
    """
    if not items:
        return []
...
def gap_delta_encode(idx: torch.Tensor) -> torch.Tensor:
    ...
    Returns the gaps as ``uint16`` if the max gap fits, else ``uint32`` — so one outlier never
    bumps the whole param to 4 B. The dtype *is* the width (no separate width needed); the
    receiver inverts with [`gap_delta_decode`].
...
def nvcomp_encode(idx: torch.Tensor) -> torch.Tensor:
    ...
    Cascaded does the delta + bit-pack internally, so callers pass raw int32 positions (no
    gap-encoding needed). The self-describing bitstream carries dtype + length, so
    [`nvcomp_decode`] needs nothing else. Cascaded is a GPU codec, so indices are moved to CUDA.
    """
    if not _NVCOMP_OK:

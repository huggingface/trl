...
class UnifiedBaseSelfDistillationTrainer(SelfDistillationMixin, _BaseTrainer, ABC):
    """Prototype base that centralizes shared self-distillation trainer lifecycle."""
    ...
    def build_training_batch(self, inputs: list[dict[str, Any]]) -> SelfDistillationBatch:
        ...
        self._validate_rollout_batch(rollout_batch)
    ...
    def _validate_rollout_batch(self, batch: SelfDistillationRolloutBatch) -> None:
        batch_size = batch.prompt_ids.size(0)
        if batch.prompt_mask.size(0) != batch_size:
            raise ValueError("`prompt_mask` must have the same batch size as `prompt_ids` in the rollout batch.")
        if batch.completion_ids.size(0) != batch_size or batch.completion_mask.size(0) != batch_size:
            raise ValueError("`completion_ids` and `completion_mask` must match the rollout batch size.")
        if batch.old_per_token_logps is not None and batch.old_per_token_logps.size(0) != batch_size:
            raise ValueError("`old_per_token_logps` must match the rollout batch size when provided.")

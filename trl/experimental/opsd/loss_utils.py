...
def compute_topk_self_distillation_loss(
    ...
    *,
    ...
    """Compute distillation loss on the teacher's top-k token support.

    The teacher's top-k logits define the support (the official OPSD convention, unlike SDFT's student support). The
    student distribution is projected onto the same token indices. The selected support is then either renormalized or
    augmented with a tail bucket before the divergence is computed.
    """
    teacher_logsumexp = torch.logsumexp(teacher_logits, dim=-1, keepdim=True)
    topk_teacher_logits, topk_indices = torch.topk(teacher_logits, k=distillation_topk, dim=-1)

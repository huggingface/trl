...
class OPSDTrainer(_BaseTrainer):
    """Trainer for OPSD-style on-policy self-distillation with explicit teacher prompts."""

    _tag_names = ["trl", "opsd"]
    _name = "OPSD"
    config_cls = OPSDConfig
    # docstyle-ignore
    _paper = {
    ...
    def __init__(
        ...
        if isinstance(eval_dataset, IterableDataset) or (
            ...
            raise NotImplementedError("Iterable eval datasets are not yet supported in OPSDTrainer.")

        self.teacher_context_builder = SolutionTeacherContextBuilder(self)
    ...
    def sample_rollouts(self, inputs: list[dict[str, Any]]) -> TrainingBatch:
        ...
        _, completion_ids_list = self._generate(student_prompt_ids_list)
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        if return_outputs:
            raise ValueError("The OPSDTrainer does not support returning outputs")
    ...
    def _compute_teacher_student_logits(
        ...
        forwarded on its input (prompt, ground-truth solution, and the same completion). Both sets of logits are
        aligned to the completion tokens so they can be compared position-by-position in the distillation loss.

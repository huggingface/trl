...
def get_any_diff(
    repo_path: str, commit_id: str, gitignore: Optional[List[str]] = None
) -> str:
    """
...
def clone_repo_at_commit(
    repo_url: str, commit_id: str, target_dir: Optional[str] = None
) -> str:
    """
...
def clean_repo_dir(repo_path: str):
    """Clean tempfolder"""
    # linux / macos / (windows?)
    assert repo_path.startswith("/tmp/") or repo_path.startswith(
        "/var/folders/"
    ), "For safety, repo_path must be a temporary directory"
